...
Plugin URI: http://laycher.com/?p=935
Description: WP-CodeBox++ is developed base on wp-codebox.It provides clean syntax highlighting and AJAX advanced features for embedding source code within pages or posts.Wrap code blocks with <code>&lt;pre lang="LANGUAGE" line="1" file="download.txt" colla="+"&gt;</code> <code>&lt;/pre&gt;</code>. The <code>LANG</code> is supported by wide range of popular languages syntax. The <code>FILE</code> will create code downloading attribute. <code>line="n"</code>is the starting line number, <code>colla="+/-"</code> will expand/collapse the codebox.
Author: Laycher
Version: 1.4.3
Author URI: http://laycher.com/
*/
#
...
function wp_codebox_plugin_links ($links, $file)
{
    if ($file == plugin_basename(__FILE__)) {
        $links[] = '<a href="http://laycher.com/?page_id=99">' . __('Donate', 'wp-codebox') . '</a>';
    }
    return $links;
}

function laycher_mce_init($initArray) { 
	$ext = 'pre[id|class|style|title|lang|dir|onclick|ondblclick|onmousedown|onmouseup|onmouseover|onmousemove|onmouseout|onkeypress|onkeydown|onkeyup|line|file|colla]'; 
...
add_filter('tiny_mce_before_init', 'laycher_mce_init');
